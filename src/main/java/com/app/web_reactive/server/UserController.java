 ...
 * String sql =
 * "INSERT INTO users (name, age, gender) VALUES (?, ?, ?) RETURNING id";
 ...
 * 
 * @PutMapping("/{id}")
 * public ResponseEntity<Map<String, Object>> updateUser(@PathVariable Long
 * id, @RequestBody Map<String, Object> user) {
 * logger.info("Updating user with ID: {}", id);
 * String sql =
 * "UPDATE users SET name = ?, email = ? WHERE id = ? RETURNING id, name, email"
 * ;
 * List<Map<String, Object>> result = jdbcTemplate.queryForList(sql,
 * user.get("name"), user.get("email"), id);
 * 
 * if (result.isEmpty()) {
 ...
 * String checkSql =
 * "SELECT COUNT(*) FROM user_media_relationships WHERE user_id = ?";
 * int count = jdbcTemplate.queryForObject(checkSql, Integer.class, id);
 * 
 * if (count > 0) {
 * logger.warn("Cannot delete user with ID: {} as they are connected to media",
 * id);
